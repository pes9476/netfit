...
def complete_daily_quest(request, quest_kind, quest_id):
    ...
    if quest.target_minutes and quest.target_minutes > 0:
        badge_type = BadgeAward.badge_for_minutes(quest.target_minutes)
    else:
        reward_points = quest.reward_points or 30
        if reward_points >= 100:
            badge_type = BadgeAward.GOLD
        elif reward_points >= 50:
            badge_type = BadgeAward.SILVER
        else:
            badge_type = BadgeAward.BRONZE
    ...
    award, created = BadgeAward.objects.get_or_create(
        **award_lookup,
        defaults={"badge_type": badge_type, "source": "DAILY_QUEST"},
    )
...
def respond_friend_request(request, request_id, action):
    if request.method == "POST":
        ...
        if action == "accept":
            ...
            freq.save(update_fields=["status", "responded_at", "sender_viewed"])
